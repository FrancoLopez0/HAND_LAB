...
esp32_object = Esp32Cam(
    orientation=0) if wifi_manager.is_connected(wifi_name) else None
...
class web_Cam_config(ctk.CTkFrame):
    def __init__(self, master, color="transparent", **kwargs):
        ...
        self.choice_Cam = option_Labeled(
            self, label="Choice Cam", callback=self.choice_Cam_callback, w=100, values=cams)
        self.choice_Cam.grid(row=0, column=0, padx=20)

        self.choice_Com = option_Labeled(
            self, label="COM", callback=self.choice_Com_callback, w=100)
...
class hands_Parameters_config(ctk.CTkFrame):
    def __init__(self, master, color="transparent", **kwargs):
        ...
        # self.distance = slider_Labeled(
        #     self, "Distance", color="transparent", callback=self.distance_Slider)
        # self.distance._set_Slider(cam.get_distance())
        # self.distance.grid(row=1, column=0)

        self.square = slider_Labeled(
            self, "Square", color="transparent", callback=self.square_Slider)
...
class checkboxes(ctk.CTkFrame):
    def __init__(self, master, webCam: HandTrackingController = None, **kwargs):
        ...
        self.var_Tracking = ctk.BooleanVar(value=False)
        self.tracking_On = ctk.CTkCheckBox(
            self, text="Tracking", hover=False, command=self.tracking, variable=self.var_Tracking, onvalue=True, offvalue=False)
        self.tracking_On.grid(row=0, column=0, padx=10, pady=10)

        self.var_Show_grid = ctk.BooleanVar(value=False)
        self.show_Grid = ctk.CTkCheckBox(self, text="Show Grid", hover=False,
                                         command=self.show_grid, variable=self.var_Show_grid, onvalue=True, offvalue=False)
...
class project_Config(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.center = ctk.CTkButton(
            self, command=self.center_cam, text="CENTER")
...
class web_Cam(ctk.CTkFrame):
    ...
    def update_frame(self):
        ...
        if ret:
            cam._program_()
            # cam_img = cv2.flip(cam.frame_0, 1)
